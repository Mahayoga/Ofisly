        ...
        <div class="card-body">
            <form action="{{ route('lowongan-pekerjaan.store') }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="form-group">
                    ...
                    <input type="date" name="tanggal_post" id="tanggal_post" class="form-control @error('tanggal_post') is-invalid @enderror" value="{{ old('tanggal_post', date('Y-m-d')) }}" required>
                    @error('tanggal_post')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
