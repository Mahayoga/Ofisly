...
class DashboardController extends Controller
{
    public function index()
    {
        ...
        for ($i = 1; $i <= 12; $i++) {
            $tp = (int) SuratTugasPromotor::whereYear('tgl_surat_pembuatan', $year)->whereMonth('tgl_surat_pembuatan', $i)->count();
            $tm = (int) SuratTugasMandiriModel::whereYear('tgl_surat_pembuatan', $year)->whereMonth('tgl_surat_pembuatan', $i)->count();
            $tpd = (int) SuratTugasPenggantiDriverModel::whereYear('tgl_surat_pembuatan', $year)->whereMonth('tgl_surat_pembuatan', $i)->count();

            $totalSromotorBulanan[] = $tp;
            $mandiriBulanan[]  = $tm;
            $penggantiBulanan[] = $tpd;
            $totalBulanan[] = $tp + $tm + $tpd;
        }
