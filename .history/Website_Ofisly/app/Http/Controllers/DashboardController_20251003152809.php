...
class DashboardController extends Controller
{
    public function index()
    {
        ...
        $daftarSuratTugas = collect()
            ->merge(SuratTugasPromotor::select('id_surat_tugas_promotor', 'tgl_surat_pembuatan')->get())->map(function ($item){
                $item->type = 'Promotor';
                return $item;
            })
            ->merge(SuratTugasMandiriModel::select('id_surat_tugas', 'tgl_surat_pembuatan')->get())->map(function ($item){
                $item->type = 'Mandiri';
                return $item;
            })
            ->merge(SuratTugasPenggantiDriverModel::select('id_surat_', 'tgl_surat_pembuatan')->get())->map(function ($item){
                $item->type = 'Pengganti Driver';
                return $item;
            });
        $daftarSuratTugas = $daftarSuratTugas->sortByDesc('tgl_surat_pembuatan')->take(5);
