...
<div class="container-fluid">
  ...
  <div class="col-auto pe-1">
    <div class="card border-left-primary shadow-sm toggle-card active" 
         data-target="surat-content" 
         style="width: 180px; height: 80px; cursor: pointer;">
      <div class="card-body p-2 d-flex align-items-center">
        <i class="fas fa-file-alt text-primary"></i>
        <div class="ms-2"> <!-- Gunakan class margin-start Bootstrap -->
  ...
  <div class="col-auto ps-1">
    <div class="card border-left-success shadow-sm toggle-card" 
        ...
        <ul class="list-group list-group-flush">
          @forelse($suratTerbaru as $s)
            <li class="list-group-item">
              <div class="small text-muted">{{ \Carbon\Carbon::parse($s->tgl_surat_pembuatan)->format('d M Y') }}</div>
              <strong>{{ $s->no_surat }}</strong><br>
              <span>{{ $s->nama_kandidat }}</span><br>
              <small>Tgl Penugasan: {{ \Carbon\Carbon::parse($s->tgl_penugasan)->format('d M Y') }}</small>
              <div class="mt-2 text-end">
                <a href="{{ url('/surat-tugas/'.$s->id_surat_tugas) }}" class="btn btn-sm btn-info">Detail</a>
              </div>
            </li>
          @empty
